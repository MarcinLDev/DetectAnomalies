...
def dodaj_etykiety_ryzyka(ramka: pd.DataFrame, progi=(40, 70)) -> pd.DataFrame:
...
prog_sredni  = st.sidebar.slider("Próg średniego ryzyka (%)", 10, 60, 40, step=5)
prog_wysoki  = st.sidebar.slider("Próg wysokiego ryzyka (%)", 50, 95, 70, step=5)
...
st.subheader("📈 Czynniki o największym wpływie na ryzyko")
...
wykres_waznosc = px.bar(
    ...
    orientation="h",
    title="Najważniejsze czynniki ryzyka (model Random Forest)"
)
wykres_waznosc.update_layout(template="plotly_white",
                             paper_bgcolor="white", plot_bgcolor="white",
                             font_color="#111", margin=dict(l=80, r=20, t=60, b=40))
st.plotly_chart(wykres_waznosc, use_container_width=True, theme=None)

st.divider()

# ============ SANDBOX „CO-JEŚLI” – miernik (gauge) ============
st.subheader("🧪 Symulacja „co-jeśli”")

wartosci_typowe = dane_wyniki.median(numeric_only=True).to_dict()
c1, c2, c3, c4 = st.columns(4)
with c1: cisnienie_sim = st.slider("Ciśnienie (PSI)", 0.0, 120.0, float(wartosci_typowe.get("Pressure_PSI", 60.0)), 0.1)
with c2: przeplyw_sim  = st.slider("Przepływ (GPM)",   0.0, 260.0, float(wartosci_typowe.get("Flow_GPM", 120.0)), 0.1)
with c3: wiek_sim      = st.slider("Wiek rury (lata)", 0,   60,    int(wartosci_typowe.get("Pipe_Age_Years", 10)), 1)
with c4: predkosc_sim  = st.slider("Prędkość (ft/s)",  0.0, 12.0, float(wartosci_typowe.get("Velocity_FPS", 4.0)), 0.1)

c5, c6 = st.columns(2)
with c5: material_sim = st.selectbox("Materiał", sorted(dane_wyniki["Pipe_Material"].unique()))
with c6: gleba_sim    = st.selectbox("Korozyjność gleby", sorted(dane_wyniki["Soil_Corrosivity"].unique()))
...
wykres_gauge = go.Figure(go.Indicator(
    ...
    value=ryzyko_sim, number={'suffix': "%"}, delta={'reference': 50},
    gauge={
        'axis': {'range': [0, 100]},
        'bar': {'thickness': 0.25},
        'steps': [
            {'range': [0, prog_sredni], 'color': 'rgba(46,184,46,0.35)'},
            {'range': [prog_sredni, prog_wysoki], 'color': 'rgba(255,204,0,0.35)'},
            {'range': [prog_wysoki, 100], 'color': 'rgba(255,77,77,0.45)'}
        ],
        'threshold': {'line': {'color': "red", 'width': 4}, 'thickness': 0.75, 'value': prog_wysoki}
    },
    title={'text': "Prognozowane ryzyko awarii – scenariusz „co-jeśli”"}
))
wykres_gauge.update_layout(template="plotly_white",
                           paper_bgcolor="white", plot_bgcolor="white",
                           font_color="#111", margin=dict(l=20, r=20, t=60, b=20))
st.plotly_chart(wykres_gauge, use_container_width=True, theme=None)

st.divider()

# ============ TOP – elementy o najwyższym ryzyku (tabela neutralna) ============
...
# wykres kołowy (do PDF – nie pokazujemy na ekranie) + białe tło
